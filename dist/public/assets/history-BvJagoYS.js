import{a as M,G as B,b as H,r as k,j as e,e as K,L as R}from"./index-CqnO_Urx.js";import{u as f}from"./useQuery-CR6eZ9A1.js";import{g as _}from"./countries-DOninoG6.js";import{C as O}from"./chevron-left-BCBL6JSC.js";import{C as b}from"./chevron-right-D5bONtHq.js";import{R as Q}from"./refresh-cw-qy4SEbLM.js";const j="/assets/nodata-da225bbb_(1)_1783249133513-DaJAjPYJ.png",A="#ff7a14",D="#111827",V=(s,i,c)=>{const r=new Date(c),a=String(r.getFullYear()).slice(2),d=String(r.getMonth()+1).padStart(2,"0"),l=String(r.getDate()).padStart(2,"0"),p=String(r.getHours()).padStart(2,"0"),m=String(r.getMinutes()).padStart(2,"0"),x=String(i).padStart(4,"0");return`sdk${a}${d}${l}${p}${m}${s}${x}`},C=s=>{const i=s.sendavapayReference||s.omnipayReference||s.omnipayId||s.soleaspayReference||s.soleaspayOrderId;return i?i.startsWith("sdk")?i:`sdk${i}`:V("D",s.id,s.createdAt)},Y=s=>s.length<=6?s:`${s.slice(0,2)}****${s.slice(-4)}`,v=s=>{const i=new Date(s),c=String(i.getDate()).padStart(2,"0"),r=String(i.getMonth()+1).padStart(2,"0"),a=i.getFullYear(),d=String(i.getHours()).padStart(2,"0"),l=String(i.getMinutes()).padStart(2,"0"),p=String(i.getSeconds()).padStart(2,"0");return`${c}/${r}/${a} ${d}:${l}:${p}`},L=s=>{switch(s){case"completed":case"approved":return{label:"Paiement réussi",color:D};case"rejected":return{label:"Paiement échoué",color:"#b42318"};case"processing":return{label:"En traitement",color:"#d97706"};default:return{label:"En attente...",color:"#d97706"}}},G=s=>{switch(s.type){case"bonus":return s.description==="Bonus quotidien"?"Bonus quotidien":s.description;case"signup_bonus":return"Bonus d'inscription";case"task_reward":return"Récompense";case"commission":return"Commission";case"deposit":return"Dépôt";default:return s.description}},h=({label:s,value:i})=>e.jsxs("div",{className:"history-row",children:[e.jsx("span",{children:s}),e.jsx("span",{children:i})]}),w=({label:s,color:i})=>e.jsx("span",{className:"history-status",style:{backgroundColor:i},children:s});function te(){const{user:s,refreshUser:i}=M(),c=B(),{toast:r}=H(),[a,d]=k.useState("withdrawals"),[l,p]=k.useState(null),m=!!s?.isAdmin,x=s?_(s.country):null,u=x?.currency==="XOF"||x?.currency==="XAF"?"FCFA":x?.currency||"FCFA",{data:N=[],isLoading:F}=f({queryKey:["/api/deposits/history"]}),{data:$=[],isLoading:z}=f({queryKey:["/api/withdrawals/history"]}),{data:I=[],isLoading:P}=f({queryKey:["/api/transactions"]}),q=t=>(t.status==="pending"||t.status==="processing")&&!!(t.soleaspayReference||t.soleaspayOrderId||t.omnipayId||t.omnipayReference||t.sendavapayReference),E=async t=>{p(t);try{const n=await(await fetch(`/api/deposits/${t}/verify`,{credentials:"include"})).json();n.status==="approved"?(r({title:"Paiement confirmé",description:"Votre compte a été crédité"}),i(),c.invalidateQueries({queryKey:["/api/deposits/history"]})):n.status==="rejected"?(r({title:"Paiement échoué",description:"Le paiement a été refusé",variant:"destructive"}),c.invalidateQueries({queryKey:["/api/deposits/history"]})):r({title:"En cours",description:"Le paiement est toujours en attente"})}catch{r({title:"Erreur",description:"Impossible de vérifier le paiement",variant:"destructive"})}finally{p(null)}};if(!s)return null;const S=[...I,{id:-1,userId:s.id,type:"registration",amount:"0",description:"Inscription",createdAt:s.createdAt}].sort((t,o)=>new Date(o.createdAt).getTime()-new Date(t.createdAt).getTime()),T=a==="balance"?P:a==="deposits"?F:z;return e.jsxs("main",{className:"history-page",children:[e.jsx("style",{children:`
        .history-page {
          width: 100%;
          min-height: 100dvh;
          overflow-x: hidden;
