import{a as T,b as O,G as V,r as c,u as U,j as e,L as x,e as C,f as P}from"./index-D8xEc-B7.js";import{u as f}from"./useQuery-C4ClxQWP.js";import{u as K}from"./useMutation-D0rDVbwm.js";import{g as Q}from"./countries-DOninoG6.js";import{C as _}from"./chevron-right-Yiv5pIgC.js";const S="/assets/t%C3%A9l%C3%A9chargement_(80)_1787363581764-DIwCG-l1.png";function ee(){const{user:l,refreshUser:I}=T(),{toast:a}=O(),R=V(),[r,m]=c.useState(""),[i,g]=c.useState(null),[b,y]=c.useState(!1),[,h]=U(),v=(l?Q(l.country):null)?.currency||"XOF",s=v==="FCFA"?"XOF":v,{data:p}=f({queryKey:["/api/settings/withdrawal"],staleTime:0,refetchOnMount:!0}),d=p?.minWithdrawal??1200,w=p?.withdrawalFees??20,j=p?.withdrawalStartHour??9,N=p?.withdrawalEndHour??17,F=r?Math.floor(Number(r)*(1-w/100)):0,k=new Date().getHours(),W=k>=j&&k<N,{data:n=[],isLoading:L}=f({queryKey:["/api/wallets"],refetchOnWindowFocus:!0}),{data:M=[]}=f({queryKey:["/api/user/products"]}),E=M.some(t=>t.status==="active");c.useEffect(()=>{const t=localStorage.getItem("selectedWalletId");if(t&&n.length>0){const o=n.find(D=>D.id===parseInt(t));o&&g(o),localStorage.removeItem("selectedWalletId")}},[n]),c.useEffect(()=>{if(!i&&n.length>0){const t=n.find(o=>o.isDefault);t&&g(t)}},[n,i]);const u=K({mutationFn:async t=>(await P("POST","/api/withdrawals",t)).json(),onSuccess:()=>{a({title:"Demande envoyée",description:"Votre demande de retrait a été envoyée."}),I(),R.invalidateQueries({queryKey:["/api/withdrawals"]}),m("")},onError:t=>{if(t.data?.code==="WITHDRAWAL_PREPAYMENT_REQUIRED"&&t.data.paymentUrl){h(t.data.paymentUrl);return}a({title:"Erreur",description:t.message,variant:"destructive"})}}),z=r?Math.max(1,Math.round(Number(r)*25/100)):0,q=async()=>{if(!r||Number(r)<d){a({title:"Montant invalide",description:`Le montant minimum est de ${d} ${s}`,variant:"destructive"});return}y(!0);try{const o=await(await P("POST","/api/withdrawal-fee/prepare",{amount:Number(r)})).json();h(o.paymentUrl)}catch(t){a({title:"Paiement indisponible",description:t.message,variant:"destructive"})}finally{y(!1)}},A=()=>{if(!W){a({title:"Horaires de retrait",description:`Les retraits sont disponibles de ${j}h à ${N}h`,variant:"destructive"});return}if(!E){a({title:"Produit requis",description:"Vous devez avoir un produit actif pour effectuer un retrait",variant:"destructive"});return}if(!r||r<d){a({title:"Montant invalide",description:`Le montant minimum est de ${d} ${s}`,variant:"destructive"});return}if(!i){a({title:"Compte requis",description:"Veuillez sélectionner un compte bancaire",variant:"destructive"});return}u.mutate({amount:Number(r),walletId:i.id})};if(L)return e.jsx("div",{className:"min-h-screen bg-white flex items-center justify-center",children:e.jsx(x,{className:"w-8 h-8 animate-spin text-[#00CC2C]"})});if(!l)return null;const H=parseFloat(l?.balance||"0"),$=n.length>0;return e.jsxs("main",{className:"withdraw-reference min-h-screen bg-[#f7f4f2]",children:[e.jsx("style",{children:`
        .withdraw-reference {
          color: #151515;
