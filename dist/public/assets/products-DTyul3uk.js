import{c as k,a as z,b as C,u as P,r as q,j as e,L as l,q as u,f as I}from"./index-BSC-zZVR.js";import{u as x}from"./useQuery-DzYZ8ggk.js";import{u as R}from"./useMutation-BAeXDnob.js";import{g as A}from"./countries-DOninoG6.js";import{c as E}from"./chargepoint_1790147948102-DERXNZnU.js";import{c as S}from"./auth-chargepoint-combined-0mIqx7uW.js";import{e as M}from"./chargepoint-empty-products-DovVT3I_.js";import{p as L,a as F,b as T,c as $,d as K,e as B,f as D,g as Q}from"./images_(39)_1783210181215-CBKWgERD.js";import{C as U}from"./chevron-right-CJ7eKu47.js";import{T as O}from"./triangle-alert-OzWv7kpE.js";const V=k("ShoppingBag",[["path",{d:"M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z",key:"hou9p0"}],["path",{d:"M3 6h18",key:"d0wm0j"}],["path",{d:"M16 10a4 4 0 0 1-8 0",key:"1ltviw"}]]),m=[L,F,T,$,K,B,D,Q];function te(){const{user:o,refreshUser:h}=z(),{toast:d}=C(),[,g]=P(),[r,c]=q.useState(null),{data:f,isLoading:b}=x({queryKey:["/api/products"]}),{data:j}=x({queryKey:["/api/user/products"]}),a=R({mutationFn:async s=>{const t=await I("POST",`/api/products/${s}/purchase`,{});if(!t.ok){const N=await t.json();throw new Error(N.message||"Erreur lors de l'achat")}return t.json()},onSuccess:()=>{u.invalidateQueries({queryKey:["/api/products"]}),u.invalidateQueries({queryKey:["/api/user/products"]}),h(),c(null),d({title:"Produit acheté !",description:"Vous commencerez à recevoir des gains demain."})},onError:s=>{c(null),d({title:"Erreur",description:s.message,variant:"destructive"})}});if(!o)return null;const y=A(o.country)?.currency||"FCFA",i=s=>`${y} ${Math.round(s).toLocaleString("fr-FR")}`,p=f?.filter(s=>!s.isFree)||[],n=(j||[]).filter(s=>s.status==="active"),v=n.length,w=n.reduce((s,t)=>s+Number(t.product?.dailyEarnings||0),0);return e.jsxs("main",{className:"cp-products-page",children:[e.jsx("style",{children:`
        .cp-products-page {
          min-height: 100%;
          padding-bottom: 76px;
          background: #080808;
          color: #f9fafb;
          font-family: Inter, Arial, sans-serif;
        }
        .cp-products-shell { width: 100%; max-width: 520px; margin: 0 auto; }
        .cp-products-header {
          display: flex;
          align-items: center;
          justify-content: space-between;
          padding: 13px 18px;
          border-bottom: 1px solid #252525;
          background: #101010;
        }
        .cp-products-brand { display: flex; align-items: center; gap: 10px; }
        .cp-products-brand img {
          width: 36px; height: 36px; border: 2px solid #ff7a14;
          border-radius: 50%; object-fit: cover; background: #fff;
        }
        .cp-products-brand strong { font-size: 18px; font-weight: 900; }
        .cp-products-service {
          border: 1px solid #4a4a4a; border-radius: 10px; padding: 7px 10px;
          background: #1b1b1b; color: #fff; font-size: 12px; font-weight: 800;
        }
        .cp-products-hero {
          position: relative; min-height: 158px; overflow: hidden;
          border-bottom: 1px solid #292929; background: #161616;
        }
        .cp-products-hero img { width: 100%; height: 158px; object-fit: cover; opacity: .65; }
        .cp-products-hero::after {
          position: absolute; inset: 0;
          background: linear-gradient(90deg, rgba(8,8,8,.9), rgba(8,8,8,.15));
          content: "";
        }
        .cp-products-hero-copy {
          position: absolute; z-index: 1; top: 26px; left: 20px; max-width: 260px;
        }
        .cp-products-hero-copy h1 { margin: 0; font-size: 29px; font-weight: 950; line-height: 1; }
        .cp-products-hero-copy p { margin: 9px 0 0; color: #d1d5db; font-size: 13px; line-height: 1.35; }
        .cp-products-stats {
          display: grid; grid-template-columns: 1fr 1fr; gap: 12px;
          padding: 16px 18px 4px; background: #080808;
        }
        .cp-products-stat {
          display: flex; min-height: 84px; align-items: center; gap: 11px;
          padding: 12px; border: 1px solid #303030; border-radius: 14px;
          background: #151515;
        }
        .cp-products-stat-icon {
          display: grid; width: 42px; height: 42px; flex: 0 0 42px;
          place-items: center; border-radius: 12px; background: #ff7a14; color: #111827;
        }
        .cp-products-stat-icon svg { width: 22px; height: 22px; }
        .cp-products-stat-value { display: block; color: #fff; font-size: 18px; font-weight: 900; }
        .cp-products-stat-label { display: block; margin-top: 4px; color: #9ca3af; font-size: 11px; }
        .cp-products-section-title {
          display: flex; align-items: center; justify-content: space-between;
          padding: 18px 18px 12px;
        }
        .cp-products-section-title h2 { margin: 0; font-size: 19px; font-weight: 900; }
        .cp-products-section-title span { color: #9ca3af; font-size: 12px; }
        .cp-products-list { display: grid; gap: 14px; padding: 0 18px; }
        .cp-product-card {
          display: grid; grid-template-columns: 118px minmax(0, 1fr);
          gap: 14px; padding: 15px; border: 1px solid #303030;
          border-radius: 17px; background: #151515; box-shadow: 0 5px 18px rgba(0,0,0,.22);
        }
        .cp-product-image {
          width: 118px; height: 118px; overflow: hidden; border: 1px solid #494949;
          border-radius: 12px; background: #222;
        }
        .cp-product-image img { width: 100%; height: 100%; object-fit: cover; }
        .cp-product-info { min-width: 0; }
        .cp-product-name {
          margin: 1px 0 10px; color: #fff; font-size: 18px; font-weight: 900;
          line-height: 1.15; overflow-wrap: anywhere;
        }
        .cp-product-line {
          display: flex; justify-content: space-between; gap: 8px;
          margin-top: 6px; color: #929292; font-size: 12px;
        }
        .cp-product-line strong { color: #f3f4f6; font-weight: 800; text-align: right; }
        .cp-product-line.revenue strong { color: #ff9b50; }
        .cp-product-buy {
          grid-column: 2; justify-self: end; min-width: 126px; margin-top: -2px;
          padding: 10px 18px; border: 1px solid #ff7a14; border-radius: 999px;
          background: #ff7a14; color: #111827; font-size: 14px; font-weight: 900;
          box-shadow: 0 4px 13px rgba(255,122,20,.28);
        }
        .cp-product-buy:active { transform: translateY(1px); }
        .cp-products-empty {
          display: flex; min-height: 260px; flex-direction: column; align-items: center;
          justify-content: center; gap: 8px; border: 1px solid #303030; border-radius: 17px;
          background: #151515; color: #9ca3af; text-align: center;
        }
        .cp-products-empty img { width: 130px; height: 130px; object-fit: contain; }
        .cp-products-empty p { margin: 0; font-size: 14px; }
        .cp-products-modal-backdrop {
          position: fixed; z-index: 60; inset: 0; display: grid; place-items: center;
          padding: 18px; background: rgba(0,0,0,.78);
        }
        .cp-products-modal {
          width: 100%; max-width: 390px; padding: 20px; border: 1px solid #474747;
          border-radius: 18px; background: #171717; box-shadow: 0 20px 60px rgba(0,0,0,.55);
        }
        .cp-products-modal h2 { margin: 0; font-size: 21px; font-weight: 900; }
        .cp-products-modal p { margin: 8px 0 0; color: #a1a1aa; font-size: 13px; line-height: 1.4; }
        .cp-products-modal-warning {
          display: flex; gap: 8px; margin-top: 15px; padding: 10px;
          border: 1px solid #633737; border-radius: 10px; background: #301b1b; color: #fca5a5;
          font-size: 12px;
        }
        .cp-products-modal-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 20px; }
        .cp-products-modal-actions button {
          padding: 11px; border: 1px solid #424242; border-radius: 10px;
          background: #242424; color: #fff; font-weight: 800;
        }
        .cp-products-modal-actions button:last-child { border-color: #ff7a14; background: #ff7a14; color: #111827; }
        @media (max-width: 370px) {
          .cp-product-card { grid-template-columns: 94px minmax(0, 1fr); gap: 10px; padding: 11px; }
          .cp-product-image { width: 94px; height: 108px; }
          .cp-product-name { font-size: 16px; }
          .cp-product-line { font-size: 11px; }
          .cp-product-buy { min-width: 108px; padding: 9px 12px; font-size: 12px; }
        }
      `}),e.jsxs("div",{className:"cp-products-shell",children:[e.jsxs("header",{className:"cp-products-header",children:[e.jsxs("div",{className:"cp-products-brand",children:[e.jsx("img",{src:E,alt:"ChargePoint"}),e.jsx("strong",{children:"Produits"})]}),e.jsx("button",{className:"cp-products-service",onClick:()=>g("/service"),children:"Service"})]}),e.jsxs("section",{className:"cp-products-hero","aria-label":"Catalogue ChargePoint",children:[e.jsx("img",{src:S,alt:""}),e.jsxs("div",{className:"cp-products-hero-copy",children:[e.jsx("h1",{children:"Équipements ChargePoint"}),e.jsx("p",{children:"Choisissez un produit et commencez à recevoir vos revenus quotidiens."})]})]}),e.jsxs("section",{className:"cp-products-stats","aria-label":"Résumé des produits",children:[e.jsxs("div",{className:"cp-products-stat",children:[e.jsx("span",{className:"cp-products-stat-icon",children:e.jsx(V,{"aria-hidden":"true"})}),e.jsxs("span",{children:[e.jsx("strong",{className:"cp-products-stat-value",children:v}),e.jsx("small",{className:"cp-products-stat-label",children:"Mes équipements"})]})]}),e.jsxs("div",{className:"cp-products-stat",children:[e.jsx("span",{className:"cp-products-stat-icon",children:e.jsx("span",{"aria-hidden":"true",children:"XAF"})}),e.jsxs("span",{children:[e.jsx("strong",{className:"cp-products-stat-value",children:i(w)}),e.jsx("small",{className:"cp-products-stat-label",children:"Revenu quotidien"})]})]})]}),e.jsxs("div",{className:"cp-products-section-title",children:[e.jsx("h2",{children:"Nos produits"}),e.jsxs("span",{children:[p.length," disponibles"]})]}),e.jsx("section",{className:"cp-products-list","aria-label":"Produits disponibles",children:b?e.jsxs("div",{className:"cp-products-empty",children:[e.jsx(l,{className:"animate-spin"}),e.jsx("p",{children:"Chargement des produits..."})]}):p.length===0?e.jsxs("div",{className:"cp-products-empty",children:[e.jsx("img",{src:M,alt:""}),e.jsx("p",{children:"Aucun produit disponible pour le moment"})]}):p.map((s,t)=>e.jsxs("article",{className:"cp-product-card","data-testid":`product-card-${s.id}`,children:[e.jsx("div",{className:"cp-product-image",children:e.jsx("img",{src:m[t%m.length],alt:s.name})}),e.jsxs("div",{className:"cp-product-info",children:[e.jsx("h3",{className:"cp-product-name",children:s.name}),e.jsxs("div",{className:"cp-product-line",children:[e.jsx("span",{children:"Prix :"}),e.jsx("strong",{children:i(Number(s.price))})]}),e.jsxs("div",{className:"cp-product-line revenue",children:[e.jsx("span",{children:"Revenu quotidien :"}),e.jsx("strong",{children:i(Number(s.dailyEarnings))})]}),e.jsxs("div",{className:"cp-product-line revenue",children:[e.jsx("span",{children:"Revenu total :"}),e.jsx("strong",{children:i(Number(s.totalReturn))})]}),e.jsxs("div",{className:"cp-product-line",children:[e.jsx("span",{children:"Cycle :"}),e.jsxs("strong",{children:[s.cycleDays," jours"]})]})]}),e.jsxs("button",{className:"cp-product-buy",onClick:()=>c(s),"data-testid":`button-purchase-${s.id}`,children:["Acheter ",e.jsx(U,{"aria-hidden":"true",size:16})]})]},s.id))})]}),r&&e.jsx("div",{className:"cp-products-modal-backdrop",onClick:()=>c(null),children:e.jsxs("div",{className:"cp-products-modal",onClick:s=>s.stopPropagation(),children:[e.jsx("h2",{children:r.name}),e.jsxs("p",{children:["Après l’achat, vos revenus seront crédités quotidiennement pendant ",r.cycleDays," jours."]}),e.jsxs("div",{className:"cp-product-line",children:[e.jsx("span",{children:"Prix :"}),e.jsx("strong",{children:i(Number(r.price))})]}),e.jsxs("div",{className:"cp-product-line revenue",children:[e.jsx("span",{children:"Revenu quotidien :"}),e.jsx("strong",{children:i(Number(r.dailyEarnings))})]}),Number(o.balance||0)<Number(r.price)&&e.jsxs("div",{className:"cp-products-modal-warning",children:[e.jsx(O,{size:16}),"Solde insuffisant pour acheter ce produit."]}),e.jsxs("div",{className:"cp-products-modal-actions",children:[e.jsx("button",{onClick:()=>c(null),children:"Annuler"}),e.jsx("button",{onClick:()=>a.mutate(r.id),disabled:a.isPending,children:a.isPending?e.jsx(l,{className:"mx-auto animate-spin",size:17}):"Confirmer"})]})]})})]})}export{te as default};
